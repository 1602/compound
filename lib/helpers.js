...
function HelperSet(ctl) {
    ...
    if (ctl) {
        ...
        this.pathTo = this.path_to = ctl.compound.map.pathTo;
    ...
    this.csrfMetaTag = this.csrf_meta_tag = function() {
        return ctl && ctl.protectedFromForgery() ? [
            '<meta name="csrf-param" content="' + ctl.req.csrfParam + '"/>',
            '<meta name="csrf-token" content="' + ctl.req.csrfToken + '"/>'
        ].join('\n') : '';
    };
...
HelperSet.prototype.stylesheetLinkTag = function stylesheetLinkTag() {
    var app = this.controller.app;
    if (!paths.css || !paths.stylesheets) {
        paths.css = this.controller.app.settings.cssDirectory || '/stylesheets/';
...
HelperSet.prototype.javascriptIncludeTag = function javascriptIncludeTag() {
    ...
    if (!paths.js || !paths.javascripts) {
        paths.js = this.controller.app.settings.jsDirectory || '/javascripts/';
...
HelperSet.prototype.formTag = function(params, block) {
    console.log('Helpers formTag and formFor(with block) are deprecated, use',
...
HelperSet.prototype.fieldsFor = function (resource, formParams, block) {
    ...
    resource = resource || {};
    var self = this;
    var resourceName = resource.modelName || resource.constructor.modelName || resource.constructor.toString() || resource.toString() || false;
    ...
    var blockHelper = {
        ...
        textarea: function (name, params) {
            ...
            return genericTag('textarea', sanitizeHTML(value), {name: makeName(name), id: makeId(name)}, params);
        },
        /*
         ...
         *
         * One element of the states array should have one field at least, 'value'.
         * For the example above, the `states` array should look like this:
         *
         * [ { name: 'Option 1', _id: 'value1' }, { name: 'Option 2', _id: 'value2' }, { name: 'Option 3', _id: 'value3' } ]
         *
         * And this array will be translated to three elements like this one:
         *
        ...
        select: function (name, options, params) {
            ...
            var escapeHtml = params.hasOwnProperty('escapeHtml') ? params.escapeHtml : true;
            delete params.escapeHtml;
            ...
            for (var optionsNr in options) {
                ...
                var option_value = option[optionFieldvalue];
                var option_name = option[optionFieldname];

                delete option[optionFieldname];
                delete option[optionFieldvalue];

                var optionParameters = option;

                // Is the value in a seperate field?
                if (option_value != option_name) {
                    optionParameters.value = option_value;
                }

                if(activeValue(option_value, optionSelected)) {
                    optionParameters.selected = 'selected';
                }

                // Generate the option Tags
                innerOptions += HelperSet.prototype.option_tag(sanitizeHTML(option_name), optionParameters)
            }
            // Render the select
...
HelperSet.prototype.formFor = function formFor(resource, params, block) {
    ...
    if (resource && resource.modelName) {
        ...
        if (!params.action) {
            params.action = this.controller.app.compound.map.pathTo[utils.underscore(resource.modelName)](resource);
        }
    }
...
HelperSet.prototype.submit_tag = HelperSet.prototype.submitTag;
 ...
 * Result:
 *
 *     <input type="radio" params />
 *
 * @methodOf HelperSet.prototype
 * @author [Artem Shoobovych](https://github.com/shybovycha)
 * @param {String} innerOptions Inner html of the select tag
 * @param {Object} params Set of tag attributes
 * @param {Object} override Set params to override params in previous arg
 * @returns {String} Finalized select tag
 */
HelperSet.prototype.radioTag = function (title, name, value, params, override) {
    params = params || {};
    params['type'] = 'radio';
    params['name'] = name;
    params['value'] = value;

    return genericTag('input', title, params, override);
};

HelperSet.prototype.radio_tag = HelperSet.prototype.radioTag;
