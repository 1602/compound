...
/**
 * Return bunch of versioned stylesheets link tags with ?v=1.2.3 appended
 * The version will be pulled from app.version so make sure you update the app.version dynamically
 * This is useful for production releases so css caches can be expired on browser clients with each version increment
 * 
 * Example in ejs:
 *
 *      <%- stylesheet_link_tag_versioned('bootstrap', 'style') %>
 *
 * This returns:
 *
 *      <link media="screen" rel="stylesheet" type="text/css" href="/stylesheets/bootstrap.css?v=1.2.3" />
 *      <link media="screen" rel="stylesheet" type="text/css" href="/stylesheets/style.css?v=1.2.3" />
 *
 * @param {String} stylesheet filename.
 * @return {String} HTML code to the stylesheets in the parameters
 */
HelperSet.prototype.stylesheetLinkTagVersioned = function stylesheetLinkTagVersioned() {
    var args;
    var app = this.controller ? this.controller.app : null;
    var version = app.version || "not_set";
    ...
    mergeFiles(app, 'stylesheets', args).forEach(function(file) {
        ...
        // there should be an option to change the /stylesheets/ folder
        var href = checkFile(app, 'css', file);
        links.push(genericTagSelfclosing('link', options, {href: href + '?v=' + version}));
    });
    return links.join('\n    ');
};
HelperSet.prototype.stylesheet_link_tag_versioned = HelperSet.prototype.stylesheetLinkTagVersioned;
...
/**
 * Generates set of versioned javascript includes composed from arguments with ?v=1.2.3 appended
 * The version will be pulled from app.version so make sure you update the app.version dynamically
 * This is useful for production releases so js caches can be expired on browser clients with each version increment
 *
 * Example in ejs:
 *
 *     <%- javascript_include_tag_versioned('rails', 'application') %>
 *
 * This returns:
 *
 *     <script type="text/javascript" src="/javascripts/rails.js?v=1.2.3"></script>
 *     <script type="text/javascript" src="/javascripts/application.js?v=1.2.3"></script>
 *
 * @param {String} script filename.
 * @return {String} the generated &lt;script&gt; tags
 */
HelperSet.prototype.javascriptIncludeTagVersioned = function javascriptIncludeTagVersioned() {
    var helpers = this;
    var args;
    var app = this.controller ? this.controller.app : null;
    var version = app.version || 'not_set';
    ...
    mergeFiles(app, 'javascripts', args).forEach(function(file) {
        ...
        var href = checkFile(app, 'js', file);
        delete options.src;
        scripts.push(helpers.tag('script', '', options, {src: href + '?v=' + version}));
    });
    return scripts.join('\n    ');
};
HelperSet.prototype.javascript_include_tag_versioned = HelperSet.prototype.javascriptIncludeTagVersioned;
