...
function HelperSet(ctl) {
    ...
    if (ctl) {
        ...
        this.pathTo = this.path_to = ctl.railway.routeMapper.pathTo;
        this.t = ctl.t;
    }
        ...
        for (var prop in resource.errors) {
            if (resource.errors.hasOwnProperty(prop)) {
                ...
                resource.errors[prop].forEach(function (msg) {
                    out += genericTag('li', railway.utils.camelize(prop, true) + ' ' + msg, {class: 'error-message'});
                });
                out += '</ul>';
            }
        }
 ...
 * @namespace
 */
HelperSet.prototype.fieldsFor = function (resource, block) {
...
HelperSet.prototype.formFor = function formFor(resource, params, block) {
    ...
    if (resource && resource.modelName) {
        ...
        if (!params.action) {
            params.action = this.controller.app.railway.routeMapper.pathTo[railway.utils.underscore(resource.modelName)](resource);
        }
    }

    this.formTag(params, function () {
        arguments.callee.buf = buf;
        if (block) self.fieldsFor(resource, block);
    });
};
