...
HelperSet.prototype.fieldsFor = function (resource, formParams, block) {
    ...
    var blockHelper = {
        ...
        button: function (name, params) {
            return genericTag('button', name, params);
        },
        ...
        select: function (name, options, params) {
            ...
            for (var optionsNr in options) {
                ...
                if (activeValue(option[optionFieldvalue], optionSelected, options.matcher)) {
                    optionParameters.selected = 'selected';
                }

                // Generate the option Tags
                innerOptions += HelperSet.prototype.optionTag(sanitizeHTML(option[optionFieldname]), optionParameters)
            }
            // Render the select
...
HelperSet.prototype.buttonTag = function (text, params) {
    return genericTag('button', text, params);
};
...
HelperSet.prototype.icon = function (type, params) {
    return genericTag('i', '', {class: 'icon-' + type}, params) + ' ';
};
