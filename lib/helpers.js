...
function HelperSet(ctl) {
    ...
    if (ctl) {
        ...
        this.pathTo = this.path_to = ctl.compound.routeMapper.pathTo;
    ...
    var blockHelper = {
        ...
        select: function (name, options, params) {
            ...
            for (var optionsNr in options) {
                ...
                innerOptions += HelperSet.prototype.option_tag(option_name, optionParameters)
            }
            // Render the select
...
HelperSet.prototype.formFor = function formFor(resource, params, block) {
    ...
    if (resource && resource.modelName) {
        ...
        if (!params.action) {
            params.action = this.controller.app.compound.routeMapper.pathTo[utils.underscore(resource.modelName)](resource);
        }
    }
 ...
 * @returns {String} Finalized select tag
 */
HelperSet.prototype.radio_tag = function (title, name, value, params, override) {
    params = params || {};
    params['type'] = 'radio';
    params['name'] = name;
    params['value'] = value;
