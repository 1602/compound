...
exports.stylesheet_link_tag = function () {
    ...
    args.forEach(function (file) {
        delete options.href;
        links.push(generic_sc_tag('link', options, {href: "/stylesheets/" + file + ".css" + (app.settings.env !== 'production' ? '?' + (new Date).getTime() : '')}));
    });
    return links.join('\n    ');
};
...
exports.javascript_include_tag = function () {
    ...
    var scripts = [];
    args.forEach(function (file) {
        var href = file.match(/^https?:\/\//) ? file : "/javascripts/" + file + ".js" + (app.settings.env !== 'production' ? '?' + (new Date).getTime() : '');
        delete options.src;
