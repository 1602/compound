...
ControllerBrigde.uniCaller = function (controller, action) {
    var ctl;
    if (controller) {
        ctl = ControllerBrigde.loadController(controller);
        if (action && !ctl.respondTo(action)) {
            console.log('WARNING: controller %s does not respond to action %s', controller, action);
        }
    }
    return function (req, res) {
        if (!ctl) {
            ctl = ControllerBrigde.loadController(req.params['controller']);
        }
