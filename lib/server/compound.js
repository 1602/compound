...
CompoundServer.prototype.init = function () {
    ...
    if (compound.app.enabled('clientside')) {
        ...
        var b = browserify();
        b.fake('express', __dirname + '/../client/application.js');
        b.fake('fs', __dirname + '/../client/fs.js');
        b.fake('util', __dirname + '/../client/util.js');
        b.add(compound.app.root + '/client-side.js');
        result.forEach(function (f) {
            b.add(f);
        });
        b.bundle({insertGlobals: true, detectGlobals: false, ignoreMissing: true}, tryMinify).pipe(fs.createWriteStream(compound.app.root + '/public/javascripts/compound.js'));
    }
