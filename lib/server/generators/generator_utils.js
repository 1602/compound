  ...
  if(utils.existsSync(fileDestPath)) {
    sys.puts($('exists').bold.grey + '  ' + displayFileName);
      ...
      try {
          ...
          sys.puts($('failed').bold.red + '  ' + displayFileName);
      }
  }
};
...
GeneratorUtilities.prototype.removeDirectory = function(dir, callback) {
...
GeneratorUtilities.prototype.copyDirectory = function(src, dest) {
    var wrench = require('wrench');

    src = path.join(__dirname, '/../../templates/', src);
    dest = path.join(this.getRootPath(), dest);

    wrench.copyDirSyncRecursive(src, dest);

    var displayFileName = dest;

    if (this.options.appName) {
        displayFileName = path.join(this.options.appName, displayFileName);
    }

    sys.puts($('copy').bold.green + '  ' + displayFileName);
};
