...
var ControllerBridge = require('./controller_bridge');
...
function Mapper(app) {
    ...
    this.resources = function (name, params, actions) {
        ...
        function getParams(action, params) {
            ...
            if (params.as) {
                ...
                p.as = ControllerBridge.urlHelperName(glob_path + p.as);
            ...
            if (params.path && !p.as) {
                ...
                aname = ControllerBridge.urlHelperName(glob_path + aname);
    ...
    ['get', 'post', 'put', 'delete', 'all'].forEach(function (method) {
        mapper[method] = function (subpath, controller_action, before_filter, options) {
            ...
            args = args.concat(ControllerBridge.uniCaller(ns, controller, action));
            ControllerBridge.dump.push({
                helper: options.as || ControllerBridge.urlHelperName(path, action),
                method: method,
                path: path,
                file: ns + controller,
                name: controller,
            ...
            ControllerBridge.addPath(path, action);
    ...
    mapper.dump = function () {
        var result = [];
        ControllerBridge.dump.forEach(function (data) {
            result.push(data);
        });
        return result;
    };
