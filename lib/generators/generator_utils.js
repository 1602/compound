 ...
 */
GeneratorUtilities.prototype.copyTemplate = function(fileName, destFileName, variables) {
  ...
  if(utils.existsSync(fileDestPath)) {
    ...
    var templateContent = tpl.templateText(fileName, this.modelProperties);
    if (!templateContent) {
 ...
 * @param {Object} Variables
 */
GeneratorUtilities.prototype.copyKontroller = function(fileName, destFileName, variables) {

  fileName = this.replaceVariables(fileName, variables);
  destFileName = this.replaceVariables(destFileName, variables);

  var filePath = path.join(__dirname, '../../node_modules/kontroller/templates', fileName);
  var fileDestPath = path.join(this.getRootPath(), destFileName);
