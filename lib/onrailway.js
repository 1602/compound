...
exports.init = function (app) {
    ...
    // wtf???
    railway.ActionObserver.application_helper = path.existsSync(app.root + '/app/helpers/application_helper.js') ?
...
function hookSockets() {
    ...
    var socket = app.railwaySocket = require('socket.io').listen(app);

    socket.on('connection', function (client) {

        client.on('message', function (m) {
            console.log('message');
            var ctl = railway.controller.load(m.c, m.c, '');
            ctl.performSocket(client, m);
        });

        // setInterval(function () {
        //     client.send('absolutely random message');
        // }, 5000);
