...
function ActionObserver(base_path, ctl_name, ctl_file) {
    ...
    var controller = ActionObserver.loadController(ctl_file);
    try {
    ...
    this.respond_to = function (action_name) {
        return controller.actions.hasOwnProperty(action_name);
    };
    this.call = function (action_name, req, res) {

        controller.actions[action_name]();
        return;
...
ActionObserver.loadController = function (name) {
    ...
    return require('./controller').load(filename);
};
