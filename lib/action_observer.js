...
function ActionObserver(base_path, ctl_name, ctl_file) {
    ...
    var super = {
        layout: controller._render && controller._render.layout !== undef ? controller._render.layout : 'application'
    };
    ...
    this.call = function (action_name, req, res) {
        controller[action_name](req, function (action, param, params) {
...
ActionObserver.loadController = function (name) {
    var filename = name + '_controller.js',
        script = new require('vm').Script(fs.readFileSync(filename), filename),
