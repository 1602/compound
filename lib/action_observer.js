...
function ActionObserver(ctl_name, ctl_file) {
    if (!ctl_file) {
        ctl_file = ctl_name;
    }
    var controller = ActionObserver.loadController(ctl_name, ctl_file);
    try {
...
ActionObserver.loadController = function (ctl_name, ctl_file) {
    return railway.controller.load(ctl_name, ctl_file);
};
