...
function Controller (name, file, basePath) {
    ...
    if (!path.existsSync(app.root + '/app/controllers/' + this.controllerFile)) {
        this.controllerFile = file + '_controller.coffee';
        if (!path.existsSync(app.root + '/app/controllers/' + this.controllerFile)) {
            throw new Error('Controller ' + this.controllerFile + ' is not defined');
        }
    }
    ...
    this.load = function (controller) {
        var baseName = app.root + "/app/controllers/" + controller + '_controller';
        if (path.existsSync(baseName + '.coffee')) {
            runCode(baseName + '.coffee', this);
        } else {
            runCode(baseName + '.js', this);
        }
    }.bind(this);
