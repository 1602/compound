var cache = {}, fs = require('fs'),
    ...
    debug       = utils.debug,
    path        = require('path'),
    addCoverage = utils.addCoverage;

function Controller (name, file, basePath) {

    var actions = {}, beforeFilters = [], afterFilters = [],
    ...
    function runCode (filename, context) {
        cache[filename] = cache[filename] || fs.readFileSync(filename);
        var code = cache[filename].toString();
        code = addCoverage(code, filename);
        var script = new require('vm').Script(code, filename);
