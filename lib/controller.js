...
function Controller (name, file, basePath) {
    ...
    // init layout (sync!)
    baseLayout = layout = path.existsSync(app.root + '/app/views/layouts/' + file + '_layout.' + app.settings['view engine']) ? file : 'application';
    ...
    this.perform = function (actionName, req, res) {
        ...
        if (req.body) {
            ...
            debug($('Body:  ').bold + JSON.stringify(filteredBody));
        }
