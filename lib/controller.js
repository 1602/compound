...
function Controller(name, file, basePath) {
    ...
    this.controllerName = name; // just name: `posts` for Posts controller
    this.controllerClassName = camelize(file.replace(/\//g, '_') + '_controller', true);
    this.controllerFullName = file; // name with path `admin/users` for Users controller inside admin namespace
    ...
    // called when received socket message
    this.performSocket = function (client, message) {
        // message protocol:
        // {
        //   c: 'controllerName',
        //   a: 'actionName', -- 'main' by default (if omitted)
        //   m: messageObject
        // }
        this.client = client;
        this.message = message.m;
        this.actionName = message.a || 'main';
        this.request = this.req = {railwaySocket: true, message: message, client: client};
        this.response = this.res = {railwaySocket: true, client: client};

        log('');
        log($((new Date).toString()).blue);
        log($('SOCKET').bold, 'controller:', $(this.controllerFullName).green, 'action:', $(this.actionName).blue);

        this._runAction(message.a || 'main');
    };

    // called when received regular http request (non-socket.io)
    this.perform = function (actionName, req, res) {
        ...
        this._runAction(actionName);
    };

    this._runAction = function (actionName) {
        var ctl = this;
        var queue = [];
