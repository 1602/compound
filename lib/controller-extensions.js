...
Controller.prototype.render = function(arg1, arg2) {
    ...
    if (layout) {
        res.render(compound.structure.views[file], locals, function (err, html) {
            if (err) {
                self.next(err);
            } else {
                locals.body = html;
                res.render(compound.structure.views[layout], locals);
                if (inAction) {
                    self.next();
                }
            }
        ...
        res.render(compound.structure.views[file], locals);
        if (inAction) {
            self.next();
        }
    }
