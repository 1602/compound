...
Controller.prototype.render = function (arg1, arg2) {
	...
	log("+++");
    if (layout) {
		log(format("+++1 %s %s", file, layout));
		/*
		var rContents = format("extends ../%s\n",layout)+require("fs").readFileSync(path.join("/home/dasher/Development/workspaces/coinage/app/views/",file+".jade"), "utf8");
		var jOptions = { filename: path.join("/home/dasher/Development/workspaces/coinage/app/views/",file+".jade"),
			pretty: true,
			compileDebug: true 
		};
		//locals.template = path.join("/home/dasher/Development/workspaces/coinage/app/views/",layout);
		rContents = require("jade").compile(rContents, jOptions);
		log(rContents);
		rContents = rContents(locals);
		log(rContents);
		res.send(rContents);
		*/
    ...
    function render(file, params, send) {
    	log(format("Render: %s with view %s", file, view));
...
Controller.prototype.layout = function layout(l) {
    var railway = this.railway;
    var app = railway.app;
    //console.log("railway:",railway, this);
