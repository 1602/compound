...
Controller.prototype.render = function render(view, params, callback) {
    ...
    if (!view) view = self.actionName;
    if (!params) params = {};

    params.request = self.req;
    params.params = self.params;

    self.prepareViewContext();
    ...
    if (!layout) {
        return self.renderFile(views[file]);
    }

    self.renderFile(views[file], function(err, html) {
        if (err) {
            return self.next(err);
        }
        self.viewContext.body = html;
        self.renderFile(views[layout]);
    });
...
Controller.prototype.prepareViewContext = function prepareViewContext() {
...
Controller.prototype.renderFile = function renderFile(filename, callback) {
    if (callback) {
        this.res.render(filename, this.locals, callback);
    } else {
        this.res.render(filename, this.locals);
