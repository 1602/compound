...
Controller.prototype.render = function(arg1, arg2) {
    ...
    var file = path.normalize(this.controllerName + '/' + view);
    ...
    var filename = compound.structure.views[file];
    if (!filename) {
        throw new Error('View ' + file + ' not found');
    }
    if (layout) {
        res.render(filename, locals, function (err, html) {
            if (err) {
                self.next(err);
            } else {
                locals.body = html;
                var layoutViewFile = compound.structure.views[layout];
                if (!layoutViewFile) {
                    throw new Error('Layout ' + layout + ' not found');
                }
                res.render(layoutViewFile, locals);
                if (inAction) {
                    self.next();
                }
            }
...
function safeMerge(mergeWhat) {
    ...
    Array.prototype.slice.call(arguments).forEach(function(mergeWith, i) {
        ...
        for (var key in mergeWith) {
            if (!mergeWith.hasOwnProperty(key) || key in mergeWhat) continue;
