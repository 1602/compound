...
Controller.prototype.render = function render(view, params, callback) {
    ...
    var device = this.context.req.device || null;

    // if it's an ajax request then just respond immediately with json
    if (self.req.isAjax) {
        return self.res.send(cleanParams(params));
    }
    ...
    if ((device && device.type == 'phone') || (device && device.type == 'tablet')) {
        view = '_mobile_' + view;
    }
    ...
    if ((device && device.type == 'phone') || (device && device.type == 'tablet')) {
        layout = layout ? 'layouts/_mobile_' + layout : false;
    } else {
    ...
    /*
    ** Removes unecessary params from being sent on a json response
     */
    function cleanParams(p) {
        for (var key in p) {
            if (p.hasOwnProperty(key)) {
                if (typeof p[key] === "function") delete p[key];
            }
            if (typeof p[key] === 'object' && key == 'user') {
                var err = new Error('You cannot pass a user object as a parameter into render() for security reasons. Pass only the info you need from the user object');
                return self.next(err);
            }
            if (key == 'apiKey') {
                var err = new Error('apiKey detected in render() params. Aborting because of potential information leakage.');
                return self.next(err);
            }
            if (key == 'api_key') {
                var err = new Error('api_key detected in render() params. Aborting to prevent leakage.');
                return self.next(err);
            }
            if (key == 'password') {
                var err = new Error('password detected in render() params. Aborting to prevent leakage.');
                return self.next(err);
            }
            if (key == 'passwordHash') {
                var err = new Error('passwordHash detected in render() params. Aborting to prevent leakage.');
                return self.next(err);
            }
            if (key == 'password_hash') {
                var err = new Error('password_hash detected in render() params. Aborting to prevent leakage.');
                return self.next(err);
            }

            // deep test objects
            if (typeof p[key] === 'object') {
                p[key] = cleanParams(p[key]);
            }
        }
        return p;
    }
