...
Controller.prototype.renderSection = function renderSection(name, view, params, callback) {
    var start = Date.now();
    var self = this;
    var compound = self.compound;
    var app = compound.app;
    var structure = compound.structure;
    var views = structure.views;
    var inAction = self.context.inAction;
    ...
    ensureFlash(self.req);

    var file = getViewFilename();

    compound.emit('renderSection', vc, self, file);

    file = calcView(compound, file, params);

    self.renderViewSection(file, function(err, html) {
        if (err) return self.next(err);
        if(self.locals.sections === undefined) self.locals.sections = {};
        self.locals.sections[name] = html;
        if (self.logger) {
            self.logger.emit('renderSection', file , Date.now() - start);
        }
    });
...
Controller.prototype.renderViewSection = function renderViewSection(view, callback) {
    var filename = this.compound.structure.views[view];
    if (!filename) {
        var err = new Error('Template ' + view + ' not found');
        if (callback) {
