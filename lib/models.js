...
function prepareContext(exportModels, defSchema, done) {
    ...
    ctx.describe = ctx.define = function (className, callback) {
        ...
        models[cname] = {};
        callback && callback();
        return global[cname] = app.models[cname] = ctx[cname] = exportModels[cname] = (schema || defSchema).define(className, models[cname]);
    };
...
exports.loadModels = function (cb) {
    ...
    fs.readdirSync(app.root + '/app/models/').forEach(function (file) {
        if (file.match(/^[^\.].*?\.(js|coffee)$/)) {
            var filename = app.root + '/app/models/' + file;
