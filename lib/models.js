...
exports.init = function (callback) {
    var result = {}, context, config, wait = 0;

    try {
        jugglingdb = require('jugglingdb');
    } catch (e) {
        console.log('jugglingdb dependency removed from railwayjs, please add it directly to you project, you can remove it fully in next release (in order to proceed with customSchema');
        return;
    }
    ...
    try {
        config = JSON.parse(fs.readFileSync(app.root + '/config/database.json', 'utf-8'))[app.set('env')];
    } catch (e) {
