...
module.exports.init = function(rw, args) {
    ...
    function initGenerator(args) {
        ...
        if (options.coffee) {
            ...
            createFile('app/controllers/application.js',
                'var ApplicationController = module.exports = function ApplicationController(init) {\n  init.before(\'protect from forgery\', function (c) {\n    ' +
                'c.protectFromForgery(\'' + secret + '\');  \n});\n};');
        }
        createFileByTemplate('config/environment',
            'config/environment',
            [replaceViewEngine, replacePrependMiddleware, replaceCssEngine]);
        ...
        // assets
        createFileByTemplate('app/assets/coffeescripts/application.coffee', 
            'assets/application.coffee')

        if (options.css === 'sass') {
            createFileByTemplate('app/assets/stylesheets/application.sass', 
              'assets/application.sass');
        } else if (options.css === 'less') {
            createFileByTemplate('app/assets/stylesheets/application.less', 
              'assets/application.less');
        } else {
            createFileByTemplate('app/assets/stylesheets/application.styl', 
              'assets/application.styl');
        }
        ...
        createFileByTemplate('public/images/compound.png',
            'compound.png');
        ...
        createFileByTemplate('public/javascripts/rails.js',
            'rails.js');
        createFileByTemplate('public/javascripts/application.js',
            'public/application.js');
        createFileByTemplate('config/autoload',
            'autoload', replaceViewEngine);
        ...
        createFile('README.md', '## Welcome to CompoundJS');

        createFileByTemplate('package.json',
            'package.json', [replaceAppname, replaceViewEngine, replaceCssEngine]);
    ...
    function modelGenerator() {
        ...
        function replaceModel(code) {
          return code.replace(/ModelName/ig, camelize(model, true));
        }
        createFileByTemplate('app/models/' + model + fileExtension, 'model' + fileExtension, replaceModel);
    ...
    function controllerGenerator() {
        ...
        // helper
        filename = 'app/helpers/' + controller + '_helper' + fileExtension;
        createFileByTemplate(filename, 'helper' + fileExtension);
    ...
    function crudGenerator(args) {
        ...
        var fileExtension = options.coffee ? '.coffee' : '.js';
        createFile('app/controllers/' + models + (options.coffee ? '_controller' : '') + fileExtension,
        ...
        function replaceModel(code) {
            return code
                .replace(/new_model/g,
                    addParents('new_', parents, model.toLowerCase()))
                .replace(/edit_model/g,
                    addParents('edit_', parents, model.toLowerCase(), true))
                .replace(/path_to\.models/g,
                    addParents('path_to.', parents, models))
                .replace(/path_to\.model/g,
                    addParents('path_to.', parents, model.toLowerCase(), true))
                .replace(/(path_to\..*?\()model/g,
        ...
        // helper
        createFileByTemplate('app/helpers/' + models + '_helper' + fileExtension,
    ...
    function controllerCode(model, attributes) {
        ...
        code = code
            ...
            .replace(/models/g, models)
            .replace(/model/g, model.toLowerCase())
            .replace(/Models/g, camelize(models, true))
            .replace(/Model/g, camelize(model, true))
    ...
    function replaceCssEngine(template) {
        return template.replace(/CSSENGINE/g, options.css || 'stylus');
    }

    function replacePrependMiddleware(template) {
        var mw = [];
        mw.push('app.use(compound.assetsCompiler.init());');
        if (options.stylus) {
            if (options.coffee) {
