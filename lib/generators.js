...
(function () {
    ...
    function initGenerator(args) {
        ...
        createFileByTemplate('package.json', 'package' + (db === 'mongoose' ? '_mongo' : '') + '.json', [replaceAppname, replaceViewEngine]);
    ...
    function modelGenerator(args) {
        ...
        createFile('app/models/' + model + fileExtension, code);
        return result;
    }
    ...
    function crudGenerator(args) {
        ...
        if (driver === 'mongoose' || driver === 'riak') {
            createFileByTemplate('test/controllers/' + models + '_controller_test', 'crud_controller_test_' + driver, replaceModel);
        } else {
     ...
     */
     ...
     function controllerCode(model, driver, result) {
        ...
        var code;
        try {
            code = fs.readFileSync(__dirname + '/../templates/crud_controller_' + driver + fileExtension);
        } catch (e) {
            code = fs.readFileSync(__dirname + '/../templates/crud_controller_redis' + fileExtension);
        }
        code = code
            .toString('utf8')
