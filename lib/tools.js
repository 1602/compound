 ...
 *
 * @param {Railway} railway - railway app.
 * @param {Array} args - command line args array.
 *
 * @return {Boolean} true.
 */
function routes(railway, args) {
    ...
    dump.forEach(function(data) {
        ...
        var filtered = !filter || filter === method ||
 ...
 *
 */
function railwayConsole(railway, args) {
    var ctx = require('repl').start('railway> ').context;

    ctx.reload = function() {
        global.models = {};
        ctx.app = railway.app;
        // ctx.app.reloadModels();
...
exports.dbconsole = function dbconsole(railway, args) {
